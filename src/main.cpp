...
struct TreeNode {
    ...
    TreeNode(int lbl, int i) : label(lbl), idx(i), featureIndex(-1), threshold(0.0), left(nullptr), right(nullptr) {}
};
...
TreeNode* buildTree(const vector<DataPoint>& data) {
    ...
    if (labelCount.size() == 1) {
        return new TreeNode(data[0].label, nodeIdx++);
    }
    ...
    if (data.empty() || data[0].features.empty()) {
        ...
        return new TreeNode(majorityLabel, nodeIdx++);
    }
    ...
    TreeNode* node = new TreeNode(-1, nodeIdx++);
